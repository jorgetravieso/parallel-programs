...
void printMatrix(datatype * matrix, int x , int y )
{
    int i,j;
    // Moves Row
    for( i = 0; i < x ; i++ )
...
void writeFile( datatype *matrix, int x1, int y1)
{
...
void CannonAlgorithm(int n, datatype *a, datatype *b, datatype *c, MPI_Comm comm) 
{ 
      int i; 
      int nlocal; 
...
int main(int argc, char* argv[])
{
  ...
  CannonAlgorithm(n, sa, sb, sc, comm);
